...
public class BakersFieldContentProvider extends ContentProvider {
    ...
    private static final String TAG = BakersFieldContentProvider.class.getName();
    ...
    public static UriMatcher buildUriMatcher() {
        ...
        // Add URI matches
        uriMatcher.addURI(BakersFieldContract.AUTHORITY, BakersFieldContract.PATH_RECIPES, RECIPES);
        uriMatcher.addURI(BakersFieldContract.AUTHORITY, BakersFieldContract.PATH_RECIPES + "/#", RECIPE_WITH_ID);
        return uriMatcher;
    }

    // Member variable for a BakersFieldDbHelper that's initialized in the onCreate() method
    private BakersFieldDbHelper mBakersFieldDbHelper;

    @Override
    public boolean onCreate() {
        Context context = getContext();
        mBakersFieldDbHelper = new BakersFieldDbHelper(context);
        return true;
    }
    ...
    public Uri insert(@NonNull Uri uri, ContentValues values) {
        final SQLiteDatabase db = mBakersFieldDbHelper.getWritableDatabase();
        ...
        switch (match) {
            case RECIPES:
                ...
                long id = db.insert(BakersFieldEntry.TABLE_NAME, null, values);
                if (id > 0) {
                    returnUri = ContentUris.withAppendedId(BakersFieldEntry.CONTENT_URI, id);
                } else {
    ...
    public Cursor query(@NonNull Uri uri, String[] projection, String selection,
        ...
        final SQLiteDatabase db = mBakersFieldDbHelper.getReadableDatabase();
        ...
        switch (match) {
            ...
            case RECIPES:
                retCursor = db.query(BakersFieldEntry.TABLE_NAME,
                        projection,
                        selection,
                        selectionArgs,
                        null,
    ...
    public int delete(@NonNull Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            ...
            case RECIPE_WITH_ID:
                ...
                plantsDeleted = db.delete(BakersFieldEntry.TABLE_NAME, "_id=?", new String[]{id});
                break;
            default:
                throw new UnsupportedOperationException("Unknown uri: " + uri);
        }
    ...
    public int update(@NonNull Uri uri, ContentValues values, String selection,
        ...
        switch (match) {
            case RECIPES:
                plantsUpdated = db.update(BakersFieldEntry.TABLE_NAME, values, selection, selectionArgs);
                break;
            case RECIPE_WITH_ID:
                if (selection == null) selection = BakersFieldEntry._ID + "=?";
                else selection += " AND " + BakersFieldEntry._ID + "=?";
